...
import com.travelSite.server.project.model.PkParamsEntity;
import com.travelSite.server.project.service.AttributesService;
import com.travelSite.server.project.service.ObjectTypesService;
...
public class DBManager {
    ...
    public List<CityBox> getCitiesList(String local) {
        AbstractApplicationContext context = new AnnotationConfigApplicationContext(SpringConfiguration.class);
        ObjectsService objectsService = (ObjectsService) context.getBean("objectsService");
        ParamsService paramsService = (ParamsService) context.getBean("paramsService");
        ...
        List<CityBox> out = new ArrayList<CityBox>();
                ...
                for (PkParamsEntity param : cityParam) {
                    ...
                    if (!cityName.equals("") && !countryName.equals("") && !latitude.equals(0.) && !longitude.equals(0.) && !thisLocal.equals("")) {
                        if (local.equals(thisLocal)) {
                            out.add(new CityBox(cityName, countryName, latitude, longitude));
                        }
                    }
                }
            }
